    ...
    return (
      <div>
        <h2 id="country-heading">
          <Translate contentKey="jhipsterFunWithFlagsApp.country.home.title">Countries</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="jhipsterFunWithFlagsApp.country.home.createLabel">Create a new Country</Translate>
          </Link>
        </h2>
        <div className="table-responsive">
          {countryList && countryList.length > 0 ? (
              ...
              <tbody>
                {countryList.map((country, i) => (
                  <tr key={`entity-${i}`}>
                    ...
                    <td>{country.flag}</td>
                    <td>{country.continent ? <Link to={`continent/${country.continent.id}`}>{country.continent.id}</Link> : ''}</td>
            ...
            <div className="alert alert-warning">
              <Translate contentKey="jhipsterFunWithFlagsApp.country.home.notFound">No Countries found</Translate>
            </div>
          )}
        </div>
      </div>
